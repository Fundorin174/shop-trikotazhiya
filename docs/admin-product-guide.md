...
> Например: Amount = `520` (текущая цена), `discount_percent` = `15` → на сайте: ~~612 ₽~~ **520 ₽**.
...
5. Введите цену — **число равно цене в рублях** (и для тканей, и для штучных товаров):
...
> ✅ **Для всех товаров:** просто введите цену в рублях. Сайт автоматически покажет это значение — за метр (для тканей) или за штуку.
